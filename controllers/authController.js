...
//===================================================================================================================//
    const signup = async (req, res) =>{
                        ...
                        if(!req.body.account) return res.status(404).json({error: "signup object is empty"});
                        ...
                        //Check if Email or username already exist
                            ...
                            if(check_email.length >= "1" || check_email2.length >= "1") return res.send({error: {message: "Email already exist"}});
                        ...
                        //Check if username already exist
                            ...
                            if(check_username.length >= "1" || check_username2.length >= "1") return res.send({error: {message: "username already exist"}});

                        //Token Time Frame
                        const tokenTimeFrame = "1h";
                        //const tokenTimeFrame = "120000";

                        //Generate token
                        const token = await authToken.generateToken({ email: account.email, username: account.username}, tokenTimeFrame);

                        //Generate random digit Code
                        const getRandomNumber = (digit) => {
                            return Math.random().toFixed(digit).split('.')[1];
                        }
                        const temp_code = getRandomNumber(4);
                        
                        //Create Temp User Oject
                        const temp_user_object = {
                                                email: account.email, username: account.username, 
                                                password: md5(account.password), token, temp_code,
                        ...
                        const temp_account_object = {
                            temp_userid: temp_user._id, user_company_name: account.user_company_name, 
                            address: account.address, city: account.city, state_province: account.state_province, 
                            postal_zip: account.postal_zip, country: account.country, skype: account.skype, 
                            tel: account.tel, interest: account.interest, agree_with_terms: account.agree_with_terms, 
...
//===================================================================================================================//
    const login = async (req, res, next) =>{
                        //Check if the payload is not empty
                        if(!req.body.users) return res.status(404).json({error: "users object is empty"});
                        ...
                        try {
                            ...
                            //find User
                            const findUser = await Users.findOne({password: md5(user.password), $or: [{email: user.email}, {username: user.username}]}, {password: 0});
                            console.log(findUser);
                            if(findUser){
                                const {email, username} = findUser;
                                const token = await authToken.generateToken({email, username}, "15m");
...
//===================================================================================================================//
    const accountActivation = async (req, res) =>{
                        //Check if the payload is not empty
                        if(!req.body.temp_user) return res.status(404).json({error: "account activation object is empty"});
                        ...
                        try {
                            ...
                            const isRegistered = await TempUser.find({token, temp_code: activationDetails.temp_code,
                                                                    email: activationDetails.email}); console.log(isRegistered);
                            ...
                            if(isRegistered.length >= 1){
                                ...
                                const {user_company_name, address, city, state_province, postal_zip, country, skype,
                                        tel, interest, agree_with_terms, isnotified} = accountdata[0];

                                // Create Account
                                const createaccount = await Accounts({userid: createuser._id, user_company_name, address, city, 
                                    state_province, postal_zip, country, skype, tel, interest, agree_with_terms, isnotified}).save();
                                ...
                                if(createuser && createaccount){
                                    ...
                                    //Delete TempAccounts
                                    await TempUser.findOneAndDelete({token, temp_code: activationDetails.temp_code});
                                    
                                    res.send({message: "Activation Successful"})
                                }

                            }else{
                                res.send({error: {message: "User does not exist or activation expired"}});
                            }
                        } catch (error) {
                            res.status(404).send({error: error.message});
                        }
...
//===================================================================================================================//
    const passwordReset = async (req, res) =>{
                        //Check if the payload is not empty
                        if(!req.body.password_reset) return res.status(404).json({error: "account activation object is empty"});
                        ...
                        try {
                            ...
                            if(passwordResetRequest.length >= 1){
                                //Reset the password
                                const resetPassword = await Users.findOneAndUpdate({email}, {$set: {password: newPassword}});
...
//===================================================================================================================//
     const forgotPassword = async (req, res) =>{
                        ...
                        //if(!req.body) return res.status(404).json({error: "email object is empty"});
                         if(!req.body.email) return res.status(404).json({error: "email object is empty"});
                        ...
                        try {
                            ...
                            const temp_token = await authToken.generateToken({email}, "1h");
                    //########################################################################################
                            // // Check if user exist
...
const mainAccounts = async (req, res, next) =>{
                        try {
                            ...
                            res.send({message: "Main Users and Accounts deletion Successful", data: {usersCollection, accountsCollection} });
                        } catch (error) {
                            res.status(404).send({error: error.message});
                        }
};
...
const tempAccounts = async (req, res, next) =>{
                        try {
                            ...
                            res.send({message: "TempUser and TempAccounts deletion Successful", data: {tempUserCollection, tempAccountsCollection} });
                        } catch (error) {
                            res.status(404).send({error: error.message});
                        }
};
...
module.exports = {
    signup, login, accountActivation, passwordReset, forgotPassword, mainAccounts, tempAccounts
}