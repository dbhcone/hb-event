...
//Import Reset Schema
import Reset from '../models/resets';
...
const accountActivation = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (isRegistered.length >= 1) {
      ...
      //Create User
      const createuser = await new Users({ email, username, password }).save();
      ...
      // Create Account
      const createaccount = await new Accounts({
...
const forgotPassword = async (req: Request, res: Response) => {
  ...
  try {
    ...
    //###########################################################################################
    const forPasswordReset = await new Reset({ email, temp_token }).save();
...
export default {tempAccounts, mainAccounts, signup, login, accountActivation, passwordReset, forgotPassword}
